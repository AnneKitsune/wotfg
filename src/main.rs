...
fn curses_render_system(_positions: &Components<Pos>, cursor: &MapCursor, layer: &LayerVisibility, curses: &mut Option<Curses>) -> SystemResult {
        ...
        let layered_cursor = ((cursor.0 >> bitshift_factor) - layered_x_start, (cursor.1 >> bitshift_factor) - layered_y_start);
        ...
        let (xmax, ymax) = (min(screen_width as u32 - outside_edge.0, square_count), min(screen_height as u32 - outside_edge.1, square_count));
        ...
        let map_offset = (
            min(max(0, layered_cursor.0 as i32 - (render_width >> 1) as i32), max(0, square_count as i32 - render_width as i32)) as u32,
            ...
            min(max(0, layered_cursor.1 as i32 - (render_height >> 1) as i32), max(0, square_count as i32 - render_height as i32)) as u32,
        );
        ...
        curses.set_color_pair(*COLOR_NORMAL);
        curses.move_rc((layered_cursor.1 + screen_offset.1 - map_offset.1) as i32,(layered_cursor.0 + screen_offset.0 - map_offset.0) as i32);
...
impl<'a> System<'a> for LayerVisibilityChangeSystem {
    ...
    fn run(&mut self, (mut input_ev, mut layer,): Self::SystemData) {
        ...
        for ev in input_ev.read(&mut self.reader.as_mut().unwrap()) {
            match ev {
                InputEvent::LayerUp => if layer.0 < 3 { layer.0 += 1 },
                InputEvent::LayerDown => if layer.0 > 0 { layer.0 -= 1 },
                _ => continue,
            }
        }
    }
...
fn main() -> amethyst::Result<()> {
    ...
    let game_data = GameDataBuilder::default()
        ...
        .with(LayerVisibilityChangeSystem::default(), "layer_move", &["curses_input"])
        .with(CursorMoveSystem::default(), "cursor_move", &["curses_input", "layer_move"])
        .with(CursesRenderSystem, "curses_render", &["cursor_move", "layer_move"]);
